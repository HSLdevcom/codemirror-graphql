...
function onMouseOver(cm, e) {
  ...
  var onHover = function onHover() {
    ...
    _codemirror2.default.off(cm.getWrapperElement(), 'mouseout', onMouseOut);
    state.hoverTimeout = undefined;
    onMouseHover(cm, box, { x: e.clientX, y: e.clientY });
  };
...
function onMouseHover(cm, box, mousePos) {
  ...
  if (render) {
    ...
    if (token) {
      ...
      if (info) {
        showPopup(cm, box, info, mousePos);
      }
    }
  }
}
...
function showPopup(cm, box, info, mousePos) {
  ...
  var topPos = mousePos.y + 10;
  if (topPos + popupHeight > window.innerHeight) {
    topPos = topPos - popupHeight - 20;
  }

  var leftPos = mousePos.x + 10;
  if (leftPos + popupWidth > window.innerWidth) {
    leftPos = leftPos - popupWidth - 20;
  }
